...
export function ProspectingSearchForm({
  ...
  const effectiveMaxLimit = Math.max(20, maxLimit);
  const limitOptions = useMemo(() => {
    const options = [20, 40, 60, 120].filter((value) => value <= effectiveMaxLimit);
    return options.length > 0 ? options : [effectiveMaxLimit];
  }, [effectiveMaxLimit]);
  const [niche, setNiche] = useState("Clínicas de estética");
  const [state, setState] = useState("TO");
  const [city, setCity] = useState("");
  const [limit, setLimit] = useState(() => Math.min(60, effectiveMaxLimit));
  const highestOption = limitOptions[limitOptions.length - 1] ?? effectiveMaxLimit;
  const selectedLimit = Math.min(limit, highestOption);

  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!niche.trim() || !state.trim()) return;
    onSearch({ niche, state, city: city.trim() || undefined, limit: selectedLimit, provider: "serpapi" });
  }
    ...
    >
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[1.1fr_0.7fr_0.9fr_0.55fr_auto]">
        ...
        <label className="block text-sm text-zinc-300">
          ...
          <span className="mt-1 block text-[11px] text-zinc-500">Limite do plano: {effectiveMaxLimit}</span>
        </label>
        <button
