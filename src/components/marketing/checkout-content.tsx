...
export function CheckoutContent({
  ...
  const selectedPlanName = useMemo(() => pricingPlans.find((plan) => plan.slug === selectedPlan)?.name ?? "CRM + Prospeccao", [selectedPlan]);
          ...
          <div className="relative grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <div className="mt-6 rounded-xl border border-white/10 bg-black/25 p-3">
                <div className="text-sm font-semibold text-white">Periodo de contratacao</div>
              ...
              <div className="mt-6 space-y-3">
                {["Escolha o plano mais adequado", "Confirme mensal, semestral ou anual", "Receba as instrucoes de pagamento", "Tenha o acesso liberado apos a confirmacao"].map((step, index) => (
            ...
            <div className="grid gap-3">
              {pricingPlans.map((plan, index) => {
                ...
                return (
                  <a
                    className={`group reveal-up relative flex items-center justify-between gap-4 overflow-hidden rounded-xl border p-4 transition hover:-translate-y-0.5 ${active ? "border-[#8B5CF6]/65 bg-[#8B5CF6]/12 shadow-xl shadow-[#8B5CF6]/10" : "border-white/10 bg-black/25 hover:border-[#8B5CF6]/50 hover:bg-[#8B5CF6]/10"}`}
                    href={`mailto:comercial@origocrm.com.br?subject=Cadastro%20${encodeURIComponent(plan.name)}%20${encodeURIComponent(billing.label)}&body=Quero%20contratar%20o%20plano%20${encodeURIComponent(plan.name)}%20no%20periodo%20${encodeURIComponent(billing.label)}.%20Valor:%20${encodeURIComponent(formatCurrency(monthlyPrice))}%20por%20usuario/mes.%20Total%20do%20periodo:%20${encodeURIComponent(formatCurrency(total))}.`}
                    key={plan.slug}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="mt-1 text-sm text-zinc-500">{formatCurrency(monthlyPrice)} / usuario / mes</div>
                      <div className="mt-1 text-xs text-zinc-600">Total {billing.shortLabel}: {formatCurrency(total)}</div>
                      {savings > 0 && <div className="mt-1 text-xs text-[#9AF0B8]">Economia de {formatCurrency(savings)} por usuario</div>}
                    </div>
              ...
              <div className="reveal-up mt-3 rounded-xl border border-[#25D366]/25 bg-[#25D366]/10 p-4 shadow-xl shadow-[#25D366]/10" style={{ "--delay": "560ms" } as CSSProperties}>
                ...
                <p className="mt-2 text-sm leading-6 text-zinc-300">
                  Plano selecionado: <span className="font-semibold text-white">{selectedPlanName}</span> no periodo <span className="font-semibold text-white">{billing.label}</span>. Clique no card para enviar sua solicitacao.
                </p>
              </div>
            </div>
          </div>
